...
png("~/Documents/Eco_Cast_Plus_2025/Figures/LIC_By_Species_Good_Spp_Updated_Points_for_Pub.png", height=6, width=5, units="in", res=300)
ggplot(Ex3, aes(y = English_Name, x = values,colour=Quarter)) +
  geom_point()+geom_jitter(width = 0.005, height=0.15)+  scale_y_discrete(limits=rev)+theme_bw()+scale_color_viridis(discrete=TRUE,option="plasma")+ theme(axis.text.y= element_text(face="bold", color="black"), axis.title.x =element_text(face="bold", color="black"), axis.text.x= element_text(face="bold", color="black"), legend.background = element_rect(fill="grey") )+
...
LIC_Annual_SPP_SSLL_Time<-Climate_Indices %>%
  filter(ind=="Thunnus albacares" | ind=="Thunnus obesus" | ind=="Isurus oxyrinchus" | ind=="Ruvettus pretiosus")

png("~/Documents/Eco_Cast_Plus_2025/Figures/LIC_By_Species_w_Trends.png", height=5.5, width=11, units="in", res=300)
ggplot(LIC_Annual_SPP_SSLL_Time, aes(y = LIC, x = Year,colour=ind)) +
  geom_point()+ geom_path() + geom_errorbar(aes(ymin = LIC-LIC_SD, ymax = LIC+LIC_SD), width = 0.2) +
theme_bw()+scale_color_viridis(discrete=TRUE)+   xlab("Year")+ylab("Local Index of Collocation")+labs(colour="Species")+theme(legend.title = element_text(size = 14), legend.text = element_text(size = 14), axis.text = element_text(size = 14),      # Axis tick labels,legend.position = "none"
