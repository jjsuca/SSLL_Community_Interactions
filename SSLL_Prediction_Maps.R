...
for (i in 1:length(Relevant_Models)){
  ...
  SSLL_Abund_Models<-readRDS(paste0("~/Documents/Eco_Cast_Plus_2025/SSLL_PA_Model/",Relevant_Models[i]))
  
  for (k in 1:50){
    Model_Estimates[,k]<-predict.gbm(SSLL_Abund_Models[[1]][[k]], Env_Vars_SSLL_Quarter_Deg,
                                     n.trees=SSLL_Abund_Models[[1]][[k]]$gbm.call$best.trees, type="response")
    print(paste("completed", k, "of 50"))}
  saveRDS(Model_Estimates, paste0("~/Documents/Eco_Cast_Plus_2025/Model_Predictions/PA/",Relevant_Species[i],"_SSLL_Abund_Model_Est.rds"))
