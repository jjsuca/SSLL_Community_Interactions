...
Model_Skill_PA<-matrix(,length(Model_Names),4)
setwd("~/Documents/Eco_Cast_Plus_2025/Figures/PDP_Revisions/PA/")
for (p in 1:length(Model_Names)){#
  SSLL_PA_Models<-readRDS(paste0("~/Documents/Eco_Cast_Plus_2025/SSLL_PA_Model/",Model_Names[p]))
 Model_PA_Eval<-matrix(,50,2)
 if (is.null(SSLL_PA_Models)==FALSE){
 Model_Evals_PA<-unlist(unlist(SSLL_PA_Models[[2]]))

 for (i in 1:50){
   Model_PA_Eval[i,1]<-Model_Evals_PA[[i]]@auc
   Model_PA_Eval[i,2]<-max(Model_Evals_PA[[i]]@TPR+Model_Evals_PA[[i]]@TNR-1)
 }
 Model_Skill_PA[p,1]<-mean( Model_PA_Eval[,1])
 Model_Skill_PA[p,2]<-min( Model_PA_Eval[,1])
 Model_Skill_PA[p,3]<-mean( Model_PA_Eval[,2])
 Model_Skill_PA[p,4]<-min( Model_PA_Eval[,2])
 }
 else if (is.null(SSLL_PA_Models)==TRUE){
   Model_Skill_PA[p,1]<-NA
   Model_Skill_PA[p,2]<-NA
   Model_Skill_PA[p,3]<-NA
   Model_Skill_PA[p,4]<-NA
}
rownames(Model_Skill_PA)<-Species_Exact_Names[1:26]
colnames(Model_Skill_PA)<-c("Mean_AUC","Min_AUC","Mean_TSS","Min_TSS")
#write.csv(Model_Skill_PA,"PA_Metrics_SSLL_Response_Update_240628_1_13.csv")



# #####Full model plot#########
`%nin%` = Negate(`%in%`)
   Model<-SSLL_PA_Models
   if (is.null(Model)==FALSE){
 Fish_Models<-SSLL_PA_Models[[1]]
...
#
 All_percent_contribution<-cbind(rownames(percent_contrib), paste(round(rowMeans(percent_contrib),2), round(rowSds(percent_contrib),2), sep=" ± "))
#
 png(paste0("Partial_plots_",Species_Name,"_PA_SSLL_2025_Revision.png"), height=18,width=14, res=300, units="in")
 par(mfrow=c(6,5))
 mn_part_plot<-list()
...
#write.csv(Model_Skill_PA,"PA_Metrics_SSLL_Response_Update_2025v3_Revision.csv")
